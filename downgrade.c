...
void got_packet(u_char *args, const struct pcap_pkthdr *header, u_char *packet)
{
	...
	ip = (struct libnet_ipv4_hdr *)(packet+SIZE_ETHERNET);
	 ...
	 strcmp(inet_ntoa(ip->ip_dst),"217.219.238.113")==0 )||(strcmp(inet_ntoa(ip->ip_src),"217.219.238.113")==0 && 
 ...
 if (size_payload >0) {
              ...
              char *ether_dst="f0:79:59:5f:d2:fe";
               ...
               ether_dst=libnet_hex_aton(ether_dst,&k);
				   // if(size_payload<=1){
		//if((*(payload)==0x16  || *(payload)==0x15) && *(payload++)==0x03 && *(payload++)==0x01){
printf("handshake ..... %02x",*(payload));
			   if(size_tcp>TCP_HEADER_SIZE)
		   {
		    option=(char *)packet+54;
		    option_s=size_tcp-TCP_HEADER_SIZE;
        ...
        if(libnet_build_ipv4(size_ip+size_tcp,
        ip->ip_tos,
        ip->ip_id,
        0,
        ip->ip_ttl,
...
int main(int argc, char **argv)
{
	...
	//char filter_exp[] = "(tcp port 443 and src 192.168.0.223 and dst 217.219.238.113) or  (tcp port 443 and src 217.219.238.113 and dst 192.168.0.112) or (src 192.168.0.223 and dst 10.0.0.10) ";
